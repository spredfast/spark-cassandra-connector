...
class CassandraLeftOuterJoinRDD[L, R] private[connector] (
    ...
    val extraBoundColumns: ColumnSelector = SomeColumns(),
  ...
  /**
   * Turns this CassandraLeftOuterJoinRDD into a factory for converting other RDD's after being serialized
   ...
   */
  private[connector] def applyToRDD(left: RDD[L]): CassandraLeftOuterJoinRDD[L, R] = {
    new CassandraLeftOuterJoinRDD[L, R](
      left,
      keyspaceName,
      tableName,
  ...
  override lazy val rowWriter = manualRowWriter match {
    case Some(_rowWriter) => _rowWriter
    case None => implicitly[RowWriterFactory[L]].rowWriter(tableDef, joinColumnNames.toIndexedSeq ++ extraBoundColumns.selectFrom(tableDef))
  }
}
